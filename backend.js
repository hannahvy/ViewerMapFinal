...
function LanguageDetector(req){
  const payload = verifyAndDecode(req.headers.authorization);
  console.log(payload)
  console.log(req.payload.comment)
  

  // arrays
  var text_input = req.payload.comment;
  var text = ["random", "123", "ni hao"];
  var lang = ["English", "Number", "Chinese"];

 // compare the inputs
  var i;
  console.log(text.length)
  for(i =0; i< text.length; i++){
    var n = text[i].localeCompare(text_input);
    console.log(n)
    if(n == 0){
      var lang_output = lang[i];
    }
  }
